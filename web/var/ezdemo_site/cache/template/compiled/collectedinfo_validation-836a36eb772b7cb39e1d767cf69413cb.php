...
// Filename:  design/standard/templates/content/collectedinfo_validation.tpl
// Timestamp: 1385723972 (Fri Nov 29 12:19:32 CET 2013)
$oldSetArray_ac74b08093d5f0eb5fabd1f57b324351 = isset( $setArray ) ? $setArray : array();
$setArray = array();
$tpl->Level++;
if ( $tpl->Level > 40 )
{
...
if ( !isset( $vars[$currentNamespace]['class'] ) )
{
    $vars[$currentNamespace]['class'] = 'warning';
    $setArray[$currentNamespace]['class'] = true;
}
...
$show2 = compiledFetchAttribute( $show1, 'processed' );
unset( $show1 );
$show1 = $show2;
...
unset( $show2 );
unset( $show2 );
$show2 = ( array_key_exists( $rootNamespace, $vars ) and array_key_exists( 'collection_attributes', $vars[$rootNamespace] ) ) ? $vars[$rootNamespace]['collection_attributes'] : null;
...
$text .= '">
            <h2>Missing or invalid input</h2>
          <ul>
          ';
unset( $show );
unset( $show );
...
if ( isset( $setArray[$currentNamespace]['class'] ) )
{
unset( $vars[$currentNamespace]['class'] );
}


$setArray = $oldSetArray_ac74b08093d5f0eb5fabd1f57b324351;
$tpl->Level--;
?>
